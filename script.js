...
class World {
  ...
  initMessage() {
    ...
    messageBody.innerHTML = `<span class="love-big">I LOVE YOU ❤️</span><br><br>Baby I know tum mujhse bahut naraz thi… aur sach me iss baar galti meri hi thi. Tumhara call pick nahi kar paya, late replies diye, aur shayad tumhe ye feel karwa diya ki mai careless hu… but trust me jaan, aisa bilkul nahi hai. Tum meri life ka woh part ho jise mai kabhi ignore kar hi nahi sakta. Kabhi kabhi situations, neend aur meri stupid habits sab kharab kar deti hain, but my feelings for you are always real, pure and constant. ❤️`;

    this.message.appendChild(closeButton);
    this.message.appendChild(messageBody);
  ...
  onPointerDown(event) {
    ...
    if (this.model) {
      ...
      clickedHeart = intersects.length > 0;
    }

    const clickNearCenter =
      Math.abs(this.pointer.x) < 0.24 && Math.abs(this.pointer.y) < 0.24;

    if (clickedHeart || clickNearCenter) {
      this.showMessage();
    }
  }
  showMessage() {
